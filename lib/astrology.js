...
// Get daily lucky numbers (3 numbers based on date + birth numerology)
export function getDailyLuckyNumbers(today, lifePathNumber) {
  const day   = today.getDate();
  const month = today.getMonth() + 1;
  const dayOfWeek = today.getDay();

  const base = reduceToSingleDigit(day + month + lifePathNumber);
  const n1 = base;
  const n2 = reduceToSingleDigit(base + dayOfWeek + 1);
  const n3 = reduceToSingleDigit(base * 2 + lifePathNumber);
...
export function getDailyReading(birthDate, today = new Date()) {
  ...
  const luckyNumbers = getDailyLuckyNumbers(today, lifePathNumber);
